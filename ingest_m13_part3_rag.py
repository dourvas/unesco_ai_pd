"""
M13 RAG Incremental Ingest Script
=================================
Module: Multimodal AI Content Creation
UNESCO Aspect 3: AI Foundations | Level: Create

Ingests ONLY the new subject_box_part3 records added April 2026:
  - 16 discipline-specific multimodal AI creation moment scenarios

Run from project root:
    python ingest_m13_part3_rag.py
"""
...
def process_m13_part3_examples(conn):
    print(f"\n{'='*70}")
    print("Processing: M13 subject_box_part3 — Multimodal Creation Scenarios")
    print(f"{'='*70}")

    cursor = conn.cursor()
    cursor.execute("SELECT id FROM modules_module WHERE code = 'M13';")
    row = cursor.fetchone()
    if not row:
        print("ERROR: M13 module record not found!")
        cursor.close()
        return False
    module_id = row[0]
    ...
    if not rows:
        print("ERROR: No subject_box_part3 records found for M13!")
        return False
    ...
    for subject_area, content_data in rows:
        text = (
            f"Module: M13 — Multimodal AI Content Creation\n"
            f"Subject: {subject_area}\n"
            f"Type: Discipline-specific multimodal AI creation moment (Video & Audio Creation — Part 3)\n"
    ...
    doc_id = store_document(
        conn,
        title='M13: Multimodal AI Content Creation - Creation Moment Scenarios by Subject (Apr 2026)',
        ...
        metadata={
            ...
            'added': 'April 2026',
            'description': 'Discipline-specific multimodal AI content creation scenarios for each subject area'
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)
    print(f"\nCOMPLETE: M13 subject_box_part3 ingested as document id={doc_id}")
    return True


def main():
    print("\n" + "="*70)
    print("M13 INCREMENTAL RAG INGEST — subject_box_part3 (April 2026)")
    print("="*70)
    ...
    try:
        ...
        if count < 16:
            print("⚠️  Less than 16 records found.")
        else:
            print("✅ All 16 subject records present")

        cursor.execute("""
            SELECT id, title, total_chunks FROM documents
            WHERE title LIKE '%M13%part3%' OR title LIKE '%M13%Creation%';
        """)
        existing = cursor.fetchall()
        cursor.close()

        if existing:
            print("\n⚠️  Found existing M13 part3 document(s):")
            for doc_id, title, chunks in existing:
                print(f"   id={doc_id}: {title} ({chunks} chunks)")
            print("   Proceeding will add a NEW document. DELETE first if replacing.\n")

        success = process_m13_part3_examples(conn)
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M13%'
            GROUP BY d.id, d.title ORDER BY d.id;
        """)
        m13_stats = cursor.fetchall()
        ...
        print("\nM13 documents in RAG corpus:")
        for title, chunks in m13_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        new_chunks = sum(c for _, c in m13_stats if 'part3' in _ or 'Creation' in _)
