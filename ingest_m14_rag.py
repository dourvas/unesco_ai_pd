"""
M14 RAG Ingest Script
====================
Module: Gamification and Immersive Learning
UNESCO Aspect 4: AI Pedagogy | Level: Create

Ingests:
  1. M14 Main Content (Universal) from modules_modulecontent
  2. M14 Subject-Specific Examples (subject_box_part2 + subject_box_part3 + subject_box_part4)

Run from project root:
    python ingest_m14_rag.py

Prerequisites:
    - .env with GEMINI_API_KEY
    - M14 module record exists in modules_module (id=19)
    - M14 main_content + subject_box records exist in modules_modulecontent
...
# ======================================================================
# M14-SPECIFIC PROCESSING
# ======================================================================

def process_module_content(conn, module_code, title):
    """Process M14 main content from database."""
    print(f"\n{'='*70}")
...
def process_module_subject_examples(conn, module_code, title):
    """Process M14 subject boxes — part2, part3, part4.

    M14 has three subject box types:
    - subject_box_part2: Gamification principle in your discipline
    - subject_box_part3: Immersive learning scenario per discipline
    - subject_box_part4: Five Roles Framework activity per discipline
    """
    print(f"\n{'='*70}")
    print(f"Processing: {title}")
    print(f"{'='*70}")
    ...
    for subject_area, content_type, content_data, _ in rows:
        type_label = {
            'subject_box_part2': 'Gamification principle in this discipline',
            'subject_box_part3': 'Immersive learning scenario',
            'subject_box_part4': 'Five Roles Framework activity',
    ...
    doc_id = store_document(
        ...
        metadata={
            ...
            'includes_five_roles_activities': True,
            'includes_immersive_scenarios': True
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)
...
def main():
    print("\n" + "="*70)
    print("M14 RAG INGEST — Gamification and Immersive Learning")
    print("="*70)
    ...
    try:
        # -- Pre-flight check ------------------------------------------
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM modules_module WHERE code = 'M14';")
        row = cursor.fetchone()
        if not row:
            print("ERROR: M14 module record not found in modules_module!")
            print("Run the M14 TAB2 SQL inserts first.")
            conn.close()
            return

        module_id = row[0]
        print(f"M14 module_id: {module_id}")
        ...
        print("\nM14 content in modules_modulecontent:")
        ...
        if process_module_content(
            conn,
            'M14',
            'M14: Gamification and Immersive Learning - Main Content'
        ):
            success_count += 1

        # -- Ingest 2: Subject Examples --------------------------------
        # M14 has 48 subject records (16 x 3 box types)
        if process_module_subject_examples(
            conn,
            'M14',
            'M14: Gamification and Immersive Learning - Subject Examples'
        ):
            success_count += 1
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M14%'
            GROUP BY d.id, d.title
            ORDER BY d.id;
        """)
        m14_stats = cursor.fetchall()
        ...
        print("\n" + "="*70)
        print("M14 INGEST COMPLETE")
        print("="*70)
        print(f"Successful: {success_count}/2")
        print()
        print("M14 documents ingested:")
        for title, chunks in m14_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        m14_chunks = sum(c for _, c in m14_stats)
        cost = (m14_chunks * 600 / 1_000_000) * 0.02
