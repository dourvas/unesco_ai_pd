...
### D10 — Article 50 attribution: visible text (CP-5 + CP-6 + CP-10 corrections)

Visible attribution line under each AI output. Implementation must (a) be a **single translatable string with named placeholders**, not concatenated `{% trans %}` fragments — Greek word order does not preserve "Generated by X on Y" structure; and (b) use **`text-base-content/70`** opacity (not `/50`) for legibility, per the same Article 50 transparency obligation that motivated the line.

```django
<p class="text-xs text-base-content/70 mt-2 ai-provenance-line">
  {% blocktrans with model=provenance.model_name generated_at=provenance.generated_at|date:"d M Y H:i" %}Generated by {{ model }} on {{ generated_at }}.{% endblocktrans %}
</p>
```

**Dispute link removed from the template tag.** Per CP-6: the existing `apps/modules/urls.py:51 path('modules/<str:code>/dispute/', ...)` is keyed on module code + a `feature_type` body parameter (RAG / RTM / DTP), not per-artefact-instance. Adding per-artefact-instance deep-linking would require URL change + view change + likely `AIOutputDispute` schema extension; out of scope for Article 50(2) machine-readability. The existing dispute UI on `tab5_reflection.html` remains accessible to users at module level. The deep-link work is deferred as a new TD entry (see §9 cross-references).

### D11 — Dispute UX affinity (CP-6 follow-up)

Dispute link is **not** emitted by `{% ai_provenance %}`. The module-level dispute UI in `tab5_reflection.html` is already discoverable to users (existing C.2.3-era UX). The per-artefact-instance dispute deep-link is logged as `TD-018 — Per-artefact-instance AI dispute deep-links` (new entry in commit 1's TD log update), deferred to post-pilot.
