"""
M1 RAG Incremental Ingest Script
=================================
Module: Understanding AI in Education
UNESCO Aspect 1: Human-Centred Mindset | Level: Acquire

Ingests ONLY the new subject_box_part3 records added April 2026:
  - 16 discipline-specific hallucination examples (one per subject)

Does NOT re-ingest main_content or existing subject_box_part2/part4
(those are already in the RAG corpus).

Run from project root:
    python ingest_m1_part3_rag.py

Prerequisites:
    - .env with GEMINI_API_KEY
    - M1 module record exists in modules_module
...
# ======================================================================
# HELPERS — identical to M5 ingest
# ======================================================================
...
# ======================================================================
# M1-SPECIFIC: subject_box_part3 only
# ======================================================================

def process_m1_part3_examples(conn):
    """
    Ingest the 16 subject_box_part3 records for M1.
    Each record is a discipline-specific hallucination example
    added April 2026 as part of the platform optimization.
    """
    print(f"\n{'='*70}")
    print("Processing: M1 subject_box_part3 — Hallucination Examples")
    print(f"{'='*70}")

    cursor = conn.cursor()

    # Get module_id
    cursor.execute("SELECT id FROM modules_module WHERE code = 'M1';")
    row = cursor.fetchone()
    if not row:
        print("ERROR: M1 module record not found!")
        cursor.close()
        return False
    module_id = row[0]
    ...
    if not rows:
        print("ERROR: No subject_box_part3 records found for M1!")
        print("Run the M1 subject_box_part3 SQL inserts first.")
        return False
    ...
    for subject_area, content_data in rows:
        text = (
            f"Module: M1 — Understanding AI in Education\n"
            f"Subject: {subject_area}\n"
            f"Type: Discipline-specific hallucination example (Critical Concepts — Part 3)\n"
    ...
    doc_id = store_document(
        conn,
        title='M1: Understanding AI in Education - Hallucination Examples by Subject (Apr 2026)',
        ...
        metadata={
            ...
            'added': 'April 2026',
            'description': 'Discipline-specific hallucination examples for Part 3 Critical Concepts'
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)

    print(f"\nCOMPLETE: M1 subject_box_part3 ingested as document id={doc_id}")
    return True
...
def main():
    print("\n" + "="*70)
    print("M1 INCREMENTAL RAG INGEST — subject_box_part3 (April 2026)")
    print("="*70)
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM documents
            WHERE title LIKE '%M1%part3%'
            OR title LIKE '%M1%Hallucination%';
        """)
        existing = cursor.fetchall()
        cursor.close()

        if existing:
            print("\n⚠️  Found existing M1 part3 document(s):")
        ...
        # -- Ingest ----------------------------------------------------
        success = process_m1_part3_examples(conn)
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M1%'
            GROUP BY d.id, d.title
            ORDER BY d.id;
        """)
        m1_stats = cursor.fetchall()
        ...
        print()
        print("M1 documents in RAG corpus:")
        for title, chunks in m1_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        # Cost estimate
        new_chunks = sum(c for _, c in m1_stats if 'part3' in _ or 'Hallucination' in _)
