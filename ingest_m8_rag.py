"""
M8 RAG Ingest Script
====================
Module: Advanced Prompt Engineering
UNESCO Aspect 3: AI Foundations | Level: Deepen

Ingests:
  1. M8 Main Content (Universal) from modules_modulecontent
  2. M8 Subject-Specific Examples (subject_box_part2 + subject_box_part4)

Run from project root:
    python ingest_m8_rag.py

Prerequisites:
    - .env with GEMINI_API_KEY
    - M8 module record exists in modules_module (id=13)
    - M8 main_content + subject_box records exist in modules_modulecontent
...
# ======================================================================
# HELPERS — identical to M3/M6 ingest
# ======================================================================
...
# ======================================================================
# M8-SPECIFIC PROCESSING FUNCTIONS
# ======================================================================
...
def main():
    print("\n" + "="*70)
    print("M8 RAG INGEST — Advanced Prompt Engineering")
    print("="*70)
    ...
    try:
        ...
        row = cursor.fetchone()
        if not row:
            print("ERROR: M8 module record not found in modules_module!")
            conn.close()
            return
        ...
        print("M8 content in modules_modulecontent:")
        ...
        if process_module_content(
            conn,
            'M8',
            'M8: Advanced Prompt Engineering - Main Content'
        ):
            success_count += 1
        ...
        if process_module_subject_examples(
            conn,
            'M8',
            'M8: Advanced Prompt Engineering - Subject Examples'
        ):
            success_count += 1
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M8%'
            GROUP BY d.id, d.title
            ORDER BY d.id;
        """)
        m8_stats = cursor.fetchall()
        ...
        print("\n" + "="*70)
        print("M8 INGEST COMPLETE")
        print("="*70)
        print(f"Successful: {success_count}/2")
        print()
        print("M8 documents ingested:")
        for title, chunks in m8_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        m8_chunks = sum(c for _, c in m8_stats)
        cost = (m8_chunks * 600 / 1_000_000) * 0.02
