#!/usr/bin/env python3
"""
Phase 2E: RAG System Testing - Windows Compatible Version
==========================================================

This version uses absolute paths to avoid import issues on Windows.
...
# === WINDOWS PATH FIX ===
# Get the absolute path to unesco_ai_pd root
...
# Verify we can find unesco_ai_pd
try:
    import unesco_ai_pd
    print(f"✅ Found unesco_ai_pd at: {unesco_ai_pd.__file__}")
except ImportError as e:
    print(f"❌ ERROR: Cannot find unesco_ai_pd module!")
    print(f"   Looking in: {project_root}")
    print(f"   Make sure this file is in: unesco_ai_pd/phase2e_testing/")
    print(f"   Error: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"❌ ERROR: Cannot import RAG system!")
    print(f"   Make sure rag_system app exists and is in INSTALLED_APPS")
    print(f"   Error: {e}")
    sys.exit(1)
