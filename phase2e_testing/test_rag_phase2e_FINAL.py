#!/usr/bin/env python3
"""
Phase 2E: RAG System Testing - For Flat Project Structure
==========================================================

Works with structure:
  unesco_ai_pd/
  ├── config/settings.py
  ├── rag_query_system.py  (directly in root)
  └── phase2e_testing/
...
# === PATH SETUP FOR FLAT STRUCTURE ===
current_file = os.path.abspath(__file__)
phase2e_dir = os.path.dirname(current_file)
project_root = os.path.dirname(phase2e_dir)
...
print(f"🔍 Phase2E directory: {phase2e_dir}")

# Django setup - settings are in config/settings.py
...
try:
    ...
    print(f"   Make sure config/settings.py exists")
    sys.exit(1)

# Import RAG system - it's directly in the root
try:
    from rag_query_system import process_reflection
    print(f"✅ RAG system imported successfully")
except ImportError as e:
    print(f"❌ ERROR: Cannot import rag_query_system!")
    print(f"   Make sure rag_query_system.py exists in: {project_root}")
    print(f"   Error: {e}")
    sys.exit(1)
...
def run_test_case(test_case):
    ...
    try:
        ...
        teacher_context = {
            'subject': test_case['subject'],
            'grade_level': test_case['grade_level'],
            'experience': 'Not specified'  # We don't track this in test cases
        }
        
        # Call RAG system
        response = process_reflection(
            reflection_text=test_case['reflection'],
        ...
        # Extract response details from process_reflection format
        feedback_text = response.get('feedback', 'N/A')
        
        # Estimate tokens from response
        # process_reflection doesn't return token counts, so we estimate
        input_tokens = len(test_case['reflection'].split()) + len(str(teacher_context).split())
        output_tokens = len(feedback_text.split())
        
        # Get actual cost from response
        total_cost = response.get('cost_eur', 0)
        # Estimate breakdown (embedding ~10%, generation ~90%)
        embedding_cost = total_cost * 0.1
        ...
        result.update({
            ...
            "response_length": len(feedback_text),
            ...
            # Costs (actual from API)
            "embedding_cost_eur": embedding_cost,
            "generation_cost_eur": generation_cost,
            "total_cost_eur": total_cost,
            
            # Retrieval info
            "chunks_retrieved": response.get('retrieved_chunks', 0),
            "top_chunk_source": 'N/A',  # process_reflection doesn't return chunk details
...
╚═══════════════════════════════════════════════════════════════════════╝
    ...
    output_dir = Path(phase2e_dir) / 'test_results'
