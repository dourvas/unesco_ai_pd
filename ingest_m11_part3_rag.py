"""
M11 RAG Incremental Ingest Script
=================================
Module: Your Voice in the AI School
UNESCO Aspect 1: Human-Centred Mindset | Level: Create

Ingests ONLY the new subject_box_part3 records added April 2026:
  - 16 discipline-specific AI leadership moment scenarios

Run from project root:
    python ingest_m11_part3_rag.py
"""
...
def process_m11_part3_examples(conn):
    print(f"\n{'='*70}")
    print("Processing: M11 subject_box_part3 — AI Leadership Moment Scenarios")
    print(f"{'='*70}")

    cursor = conn.cursor()
    cursor.execute("SELECT id FROM modules_module WHERE code = 'M11';")
    row = cursor.fetchone()
    if not row:
        print("ERROR: M11 module record not found!")
        cursor.close()
        return False
    module_id = row[0]
    ...
    if not rows:
        print("ERROR: No subject_box_part3 records found for M11!")
        return False
    ...
    for subject_area, content_data in rows:
        text = (
            f"Module: M11 — Your Voice in the AI School\n"
            f"Subject: {subject_area}\n"
            f"Type: Discipline-specific AI leadership moment scenario (Propose Change — Part 4)\n"
    ...
    doc_id = store_document(
        conn,
        title='M11: Your Voice in the AI School - Leadership Moment Scenarios by Subject (Apr 2026)',
        ...
        metadata={
            ...
            'added': 'April 2026',
            'description': 'Discipline-specific scenarios where teacher uses their voice to shape school AI practice'
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)
    print(f"\nCOMPLETE: M11 subject_box_part3 ingested as document id={doc_id}")
    return True


def main():
    print("\n" + "="*70)
    print("M11 INCREMENTAL RAG INGEST — subject_box_part3 (April 2026)")
    print("="*70)
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE title LIKE '%M11%part3%' OR title LIKE '%M11%Leadership%';
        """)
        existing = cursor.fetchall()
        cursor.close()

        if existing:
            print("\n⚠️  Found existing M11 part3 document(s):")
        ...
        success = process_m11_part3_examples(conn)
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M11%'
            GROUP BY d.id, d.title ORDER BY d.id;
        """)
        m11_stats = cursor.fetchall()
        ...
        print("\nM11 documents in RAG corpus:")
        for title, chunks in m11_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        new_chunks = sum(c for _, c in m11_stats if 'part3' in _ or 'Leadership' in _)
