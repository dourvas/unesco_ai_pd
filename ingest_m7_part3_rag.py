"""
M7 RAG Incremental Ingest Script
=================================
Module: Navigating Ethical Dilemmas in AI Use
UNESCO Aspect 2: Ethics | Level: Deepen

Ingests ONLY the new subject_box_part3 records added April 2026:
  - 16 discipline-specific academic integrity dilemma scenarios

Run from project root:
    python ingest_m7_part3_rag.py
"""
...
def process_m7_part3_examples(conn):
    print(f"\n{'='*70}")
    print("Processing: M7 subject_box_part3 — Academic Integrity Dilemmas")
    print(f"{'='*70}")

    cursor = conn.cursor()
    cursor.execute("SELECT id FROM modules_module WHERE code = 'M7';")
    row = cursor.fetchone()
    if not row:
        print("ERROR: M7 module record not found!")
        cursor.close()
        return False
    module_id = row[0]
    ...
    if not rows:
        print("ERROR: No subject_box_part3 records found for M7!")
        return False
    ...
    for subject_area, content_data in rows:
        text = (
            f"Module: M7 — Navigating Ethical Dilemmas in AI Use\n"
            f"Subject: {subject_area}\n"
            f"Type: Discipline-specific academic integrity dilemma (Academic Integrity — Part 2)\n"
    ...
    doc_id = store_document(
        conn,
        title='M7: Navigating Ethical Dilemmas - Academic Integrity Dilemmas by Subject (Apr 2026)',
        ...
        metadata={
            ...
            'added': 'April 2026',
            'description': 'Discipline-specific academic integrity dilemmas requiring principled judgment'
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)
    print(f"\nCOMPLETE: M7 subject_box_part3 ingested as document id={doc_id}")
    return True


def main():
    print("\n" + "="*70)
    print("M7 INCREMENTAL RAG INGEST — subject_box_part3 (April 2026)")
    print("="*70)
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE title LIKE '%M7%part3%' OR title LIKE '%M7%Integrity%';
        """)
        existing = cursor.fetchall()
        cursor.close()

        if existing:
            print("\n⚠️  Found existing M7 part3 document(s):")
        ...
        success = process_m7_part3_examples(conn)
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M7%'
            GROUP BY d.id, d.title ORDER BY d.id;
        """)
        m7_stats = cursor.fetchall()
        ...
        print("\nM7 documents in RAG corpus:")
        for title, chunks in m7_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        new_chunks = sum(c for _, c in m7_stats if 'part3' in _ or 'Integrity' in _)
