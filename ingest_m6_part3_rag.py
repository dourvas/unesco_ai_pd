"""
M6 RAG Incremental Ingest Script
=================================
Module: Human Accountability in AI
UNESCO Aspect 1: Human-Centred Mindset | Level: Deepen

Ingests ONLY the new subject_box_part3 records added April 2026:
  - 16 discipline-specific accountability moment scenarios

Run from project root:
    python ingest_m6_part3_rag.py
"""
...
def process_m6_part3_examples(conn):
    print(f"\n{'='*70}")
    print("Processing: M6 subject_box_part3 — Accountability Moment Scenarios")
    print(f"{'='*70}")

    cursor = conn.cursor()
    cursor.execute("SELECT id FROM modules_module WHERE code = 'M6';")
    row = cursor.fetchone()
    if not row:
        print("ERROR: M6 module record not found!")
        cursor.close()
        return False
    module_id = row[0]
    ...
    if not rows:
        print("ERROR: No subject_box_part3 records found for M6!")
        return False
    ...
    for subject_area, content_data in rows:
        text = (
            f"Module: M6 — Human Accountability in AI\n"
            f"Subject: {subject_area}\n"
            f"Type: Discipline-specific accountability moment scenario (Human Accountability — Part 3)\n"
    ...
    doc_id = store_document(
        conn,
        title='M6: Human Accountability in AI - Accountability Moment Scenarios by Subject (Apr 2026)',
        ...
        metadata={
            ...
            'added': 'April 2026',
            'description': 'Discipline-specific scenarios where teacher must override or annotate AI output'
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)
    print(f"\nCOMPLETE: M6 subject_box_part3 ingested as document id={doc_id}")
    return True


def main():
    print("\n" + "="*70)
    print("M6 INCREMENTAL RAG INGEST — subject_box_part3 (April 2026)")
    print("="*70)
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE title LIKE '%M6%part3%' OR title LIKE '%M6%Accountability%';
        """)
        existing = cursor.fetchall()
        cursor.close()

        if existing:
            print("\n⚠️  Found existing M6 part3 document(s):")
        ...
        success = process_m6_part3_examples(conn)
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M6%'
            GROUP BY d.id, d.title ORDER BY d.id;
        """)
        m6_stats = cursor.fetchall()
        ...
        print("\nM6 documents in RAG corpus:")
        for title, chunks in m6_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        new_chunks = sum(c for _, c in m6_stats if 'part3' in _ or 'Accountability' in _)
