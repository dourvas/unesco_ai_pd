"""
M6 RAG Ingest Script
====================
Module: Human Accountability in AI
UNESCO Aspect 1: Human-Centred Mindset | Level: Deepen

Follows the exact same pattern as the existing ingest.py used for M7/M12.
Ingests:
  1. M6 Main Content (Universal) from modules_modulecontent
  2. M6 Subject-Specific Examples (subject_box_part2 + subject_box_part4)

Run from project root:
    python ingest_m6_rag.py

Prerequisites:
    - .env with GEMINI_API_KEY
    - M6 module record exists in modules_module
    - M6 main_content + subject_box records exist in modules_modulecontent
    - documents + document_chunks tables exist (same schema as M1/M2/M7/M12)
"""
...
# -- API setup (mirrors existing ingest.py) ----------------------------
NEW_GENAI_API = False
try:
...
# ======================================================================
# HELPERS — copied verbatim from ingest.py
# ======================================================================
...
# ======================================================================
# M6-SPECIFIC PROCESSING FUNCTIONS
# ======================================================================

def process_module_content(conn, module_code, title):
    """Process module main content from database — same as ingest.py."""
    print(f"\n{'='*70}")
...
def process_module_subject_examples(conn, module_code, title):
    """Process subject boxes (part2 + part4) — same as ingest.py."""
    print(f"\n{'='*70}")
    print(f"Processing: {title}")
    print(f"{'='*70}")
...
def main():
    print("\n" + "="*70)
    print("M6 RAG INGEST — Human Accountability in AI")
    print("="*70)
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id FROM modules_module WHERE code = 'M6';
        """)
        row = cursor.fetchone()
        if not row:
            print("ERROR: M6 module record not found in modules_module!")
            print("Run the M6 TAB2 SQL insert first.")
            conn.close()
            return
        ...
        print("M6 content in modules_modulecontent:")
        ...
        if process_module_content(
            conn,
            'M6',
            'M6: Human Accountability in AI - Main Content'
        ):
            success_count += 1
        ...
        if process_module_subject_examples(
            conn,
            'M6',
            'M6: Human Accountability in AI - Subject Examples'
        ):
            success_count += 1
        ...
        print("\n" + "="*70)
        print("M6 INGEST COMPLETE")
        print("="*70)
        print(f"Successful: {success_count}/2")
        print()
        print("M6 documents ingested:")
        ...
        # Cost estimate
        m6_chunks = sum(c for _, c in m6_stats)
        cost = (m6_chunks * 600 / 1_000_000) * 0.02
