"""
M2 RAG Incremental Ingest Script
=================================
Module: Ethics of AI in Education
UNESCO Aspect 2: Ethics | Level: Acquire

Ingests ONLY the new subject_box_part3 records added April 2026:
  - 16 discipline-specific AI bias examples (one per subject)

Does NOT re-ingest main_content or existing subject_box_part2/part4
(those are already in the RAG corpus).

Run from project root:
    python ingest_m2_part3_rag.py

Prerequisites:
    - .env with GEMINI_API_KEY
    - M2 module record exists in modules_module
...
# ======================================================================
# HELPERS — identical to M1 ingest
# ======================================================================
...
# ======================================================================
# M2-SPECIFIC: subject_box_part3 only
# ======================================================================

def process_m2_part3_examples(conn):
    """
    Ingest the 16 subject_box_part3 records for M2.
    Each record is a discipline-specific AI bias example
    added April 2026 as part of the platform optimization.
    """
    print(f"\n{'='*70}")
    print("Processing: M2 subject_box_part3 — AI Bias Examples by Subject")
    print(f"{'='*70}")

    cursor = conn.cursor()

    # Get module_id
    cursor.execute("SELECT id FROM modules_module WHERE code = 'M2';")
    row = cursor.fetchone()
    if not row:
        print("ERROR: M2 module record not found!")
        cursor.close()
        return False
    module_id = row[0]
    ...
    if not rows:
        print("ERROR: No subject_box_part3 records found for M2!")
        print("Run the M2 subject_box_part3 SQL inserts first.")
        return False
    ...
    for subject_area, content_data in rows:
        text = (
            f"Module: M2 — Ethics of AI in Education\n"
            f"Subject: {subject_area}\n"
            f"Type: Discipline-specific AI bias example (Key Challenges — Part 3)\n"
    ...
    doc_id = store_document(
        conn,
        title='M2: Ethics of AI in Education - AI Bias Examples by Subject (Apr 2026)',
        ...
        metadata={
            ...
            'added': 'April 2026',
            'description': 'Discipline-specific AI bias examples for Part 3 Key Challenges'
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)

    print(f"\nCOMPLETE: M2 subject_box_part3 ingested as document id={doc_id}")
    return True
...
def main():
    print("\n" + "="*70)
    print("M2 INCREMENTAL RAG INGEST — subject_box_part3 (April 2026)")
    print("="*70)
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM documents
            WHERE title LIKE '%M2%part3%'
            OR title LIKE '%M2%Bias%';
        """)
        existing = cursor.fetchall()
        cursor.close()

        if existing:
            print("\n⚠️  Found existing M2 part3 document(s):")
        ...
        # -- Ingest ----------------------------------------------------
        success = process_m2_part3_examples(conn)
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M2%'
            GROUP BY d.id, d.title
            ORDER BY d.id;
        """)
        m2_stats = cursor.fetchall()
        ...
        print()
        print("M2 documents in RAG corpus:")
        for title, chunks in m2_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        # Cost estimate
        new_chunks = sum(c for _, c in m2_stats if 'part3' in _ or 'Bias' in _)
