"""
M9 RAG Ingest Script
====================
Module: AI-Enhanced Lesson Design
UNESCO Aspect 4: AI Pedagogy | Level: Deepen

Ingests:
  1. M9 Main Content (Universal) from modules_modulecontent
  2. M9 Subject-Specific Examples (subject_box_part2 + subject_box_part3 + subject_box_part4)

Run from project root:
    python ingest_m9_rag.py

Prerequisites:
    - .env with GEMINI_API_KEY
    - M9 module record exists in modules_module (id=17)
    - M9 main_content + subject_box records exist in modules_modulecontent
...
# ======================================================================
# M9-SPECIFIC PROCESSING
# ======================================================================

def process_module_content(conn, module_code, title):
    """Process M9 main content from database."""
    print(f"\n{'='*70}")
...
def process_module_subject_examples(conn, module_code, title):
    """Process M9 subject boxes — part2, part3, part4.

    M9 has three subject box types:
    - subject_box_part2: UDL principle in your subject
    - subject_box_part3: Differentiation scenarios (ESL/SEN/Advanced) per discipline
    - subject_box_part4: Flipped learning scenario per discipline
    """
    print(f"\n{'='*70}")
    print(f"Processing: {title}")
    print(f"{'='*70}")
    ...
    for subject_area, content_type, content_data, _ in rows:
        type_label = {
            'subject_box_part2': 'UDL in your subject',
            'subject_box_part3': 'Differentiation scenarios (ESL/SEN/Advanced)',
            'subject_box_part4': 'Flipped learning scenario',
    ...
    doc_id = store_document(
        ...
        metadata={
            ...
            'includes_part3_differentiation_scenarios': True
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)
...
def main():
    print("\n" + "="*70)
    print("M9 RAG INGEST — AI-Enhanced Lesson Design")
    print("="*70)
    ...
    try:
        # -- Pre-flight check ------------------------------------------
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM modules_module WHERE code = 'M9';")
        row = cursor.fetchone()
        if not row:
            print("ERROR: M9 module record not found in modules_module!")
            print("Run the M9 TAB2 SQL inserts first.")
            conn.close()
            return

        module_id = row[0]
        print(f"M9 module_id: {module_id}")
        ...
        print("\nM9 content in modules_modulecontent:")
        ...
        if process_module_content(
            conn,
            'M9',
            'M9: AI-Enhanced Lesson Design - Main Content'
        ):
            success_count += 1

        # -- Ingest 2: Subject Examples --------------------------------
        # M9 has 48 subject records (16 x 3 box types)
        if process_module_subject_examples(
            conn,
            'M9',
            'M9: AI-Enhanced Lesson Design - Subject Examples'
        ):
            success_count += 1
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M9%'
            GROUP BY d.id, d.title
            ORDER BY d.id;
        """)
        m9_stats = cursor.fetchall()
        ...
        print("\n" + "="*70)
        print("M9 INGEST COMPLETE")
        print("="*70)
        print(f"Successful: {success_count}/2")
        print()
        print("M9 documents ingested:")
        for title, chunks in m9_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        m9_chunks = sum(c for _, c in m9_stats)
        cost = (m9_chunks * 600 / 1_000_000) * 0.02
