"""
M10 RAG Ingest Script
====================
Module: AI Collaboration and Communities of Practice
UNESCO Aspect 5: Professional Development | Level: Deepen

Ingests:
  1. M10 Main Content (Universal) from modules_modulecontent
  2. M10 Subject-Specific Examples (subject_box_part2 + subject_box_part3 + subject_box_part4)

Run from project root:
    python ingest_m10_rag.py

Prerequisites:
    - .env with GEMINI_API_KEY
    - M10 module record exists in modules_module (id=18)
    - M10 main_content + subject_box records exist in modules_modulecontent
    - documents + document_chunks tables exist

Subject box types:
    - subject_box_part2: Discipline-specific shared prompt library content
    - subject_box_part3: Annotated RPE prompt example per discipline
    - subject_box_part4: CoP session design per discipline
"""
...
# ======================================================================
# M10-SPECIFIC PROCESSING
# ======================================================================

def process_module_content(conn, module_code, title):
    """Process M10 main content from database."""
    print(f"\n{'='*70}")
...
def process_module_subject_examples(conn, module_code, title):
    """Process M10 subject boxes — part2, part3, part4.

    M10 has three subject box types:
    - subject_box_part2: Discipline-specific shared prompt library content
    - subject_box_part3: Annotated RPE prompt example per discipline
    - subject_box_part4: CoP session design per discipline
    """
    ...
    for subject_area, content_type, content_data, _ in rows:
        type_label = {
            'subject_box_part2': 'Discipline-specific shared prompt library',
            'subject_box_part3': 'Annotated RPE prompt example',
            'subject_box_part4': 'CoP session design',
    ...
    doc_id = store_document(
        ...
        metadata={
            ...
            'includes_annotated_rpe_prompts': True,
            'includes_cop_session_designs': True,
        }
    )
    store_chunks(conn, doc_id, chunks, embeddings)
...
def main():
    print("\n" + "="*70)
    print("M10 RAG INGEST — AI Collaboration and Communities of Practice")
    print("="*70)
    ...
    try:
        # -- Pre-flight check ------------------------------------------
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM modules_module WHERE code = 'M10';")
        row = cursor.fetchone()
        if not row:
            print("ERROR: M10 module record not found in modules_module!")
            print("Run the M10 TAB2 SQL inserts first.")
            conn.close()
            return

        module_id = row[0]
        print(f"M10 module_id: {module_id}")
        ...
        print("\nM10 content in modules_modulecontent:")
        ...
        if process_module_content(
            conn,
            'M10',
            'M10: AI Collaboration and Communities of Practice - Main Content'
        ):
            success_count += 1

        # -- Ingest 2: Subject Examples --------------------------------
        # M10 has 48 subject records (16 x 3 box types)
        if process_module_subject_examples(
            conn,
            'M10',
            'M10: AI Collaboration and Communities of Practice - Subject Examples'
        ):
            success_count += 1
        ...
        cursor.execute("""
            ...
            WHERE d.title LIKE 'M10%'
            GROUP BY d.id, d.title
            ORDER BY d.id;
        """)
        m10_stats = cursor.fetchall()
        ...
        print("\n" + "="*70)
        print("M10 INGEST COMPLETE")
        print("="*70)
        print(f"Successful: {success_count}/2")
        print()
        print("M10 documents ingested:")
        for title, chunks in m10_stats:
            print(f"  {title}: {chunks} chunks")
        print(f"\nTotal chunks in database (all modules): {total_chunks}")
        print(f"\nEnd: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        m10_chunks = sum(c for _, c in m10_stats)
        cost = (m10_chunks * 600 / 1_000_000) * 0.02
